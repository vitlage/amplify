...
class Subscriber extends Model
{
    ...
    {
        return $this->hasMany('App\Model\EmailVerification');
    }

    public function emailVerification()
    {
        return $this->hasOne('App\Model\EmailVerification');
    }
    ...
    {
        return $this->hasMany('App\Model\SubscriberField');
    }
    ...
    {
        return $this->hasMany('App\Model\UnsubscribeLog');
    }
    ...
    public static function import($user, $list)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            // authorize
            if ($user->cannot('create', new \App\Model\Subscriber(['mail_list_id' => $list->id]))) {
                ...
                if ($valid) {
                    ...
                    foreach ($parts as $key => $value) {
                        ...
                        if (isset($headers[$key])) {
                            ...
                            if (is_object($lf)) {
                                //// save fields
                                $lfv = new \App\Model\SubscriberField(array(
                                                'field_id' => $lf->id,
    ...
    public function copy($list, $type = 'keep')
    {
        ...
        if (!is_object($copy)) {
            ...
            $copy = \App\Model\Subscriber::find($copy->id);
        } else {
            // return if keep
            if ($type == 'keep') {
                return false;
    ...
    {
        $query = \App\Model\TrackingLog::where('tracking_logs.subscriber_id', '=', $this->id);
    ...
    {
        $query = \App\Model\OpenLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'open_logs.message_id')
    ...
    {
        $query = \App\Model\ClickLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'click_logs.message_id')
            ->where('tracking_logs.subscriber_id', '=', $this->id);
