...
class Template extends Model implements Storable
{
    ...
    {
        $this->content = \App\Library\Tool::replaceTemplateUrl($this->content, $template_path);
    }
    ...
    public function copy($name, $customer = null, $admin = null)
    {
        ...
        if (file_exists($this->getStoragePath())) {
            ...
            \App\Library\Tool::xcopy($this->getStoragePath(), $copy->getStoragePath());
        }
    ...
    public static function upload($request, $asAdmin = false)
    {
        ...
        // UID is critically required by untransform()
        $template->uid = uniqid();
    ...
    {
        // @todo workaround
        // to avoid exception thrown by route(template_assets)
        if (is_null($this->uid)) {
            return;
        }
    ...
    public function untransform()
    {
        ...
        $this->content = str_replace(
            route('template_assets', ['uid' => $this->uid]).'/',
            '',
    ...
    public function transformWithoutUpdate($html, $useTrackingHost = false)
    {
        ...
        // Replace #1
        $path = join_url($host, route('template_assets', ['uid' => $this->uid], false));
    ...
    public function load($path)
    {
        ...
        \App\Library\Tool::xcopy($path, $this->getUploadDir());
    ...
    {
        \App\Library\Tool::xcopy($template->getUploadDir(), $this->getUploadDir());
