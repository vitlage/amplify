...
abstract class AbsCampaign extends Model
{
    ...
    {
        return $this->belongsToMany('App\Model\Link', 'campaign_links');
    }
    ...
    {
        return $this->hasMany('App\Model\CampaignsListsSegment');
    }
    ...
    public static function boot()
    {
        ...
        static::deleted(function ($item) {
            ...
            \App\Library\Tool::xdelete($uploaded_dir);
        });
    }
    ...
    {
        return \App\Web\Subscriber::selectRaw(DB::getTablePrefix().'list_subscriber.*, COUNT('.DB::getTablePrefix().'campaign_track_unsubscribe.id) AS openCount')
    ...
    public function copy($name)
    {
        ...
        if (file_exists($this->getStoragePath())) {
            \App\Library\Tool::xcopy($this->getStoragePath(), $newCampaign->getStoragePath());
        }
    ...
    {
        if ($this->getThumbName()) {
            return route('campaign_assets', ['uid' => $this->uid, 'name' => $this->getThumbName()]);
        } else {
    ...
    public function transformWithoutUpdate($html, $useTrackingHost = false)
    {
        ...
        // Replace #1
        $path = join_url($host, route('campaign_assets', ['uid' => $this->uid], false));
        $html = \App\Library\Tool::replaceTemplateUrl($html, $path);
    ...
    public function untransform()
    {
        ...
        $this->html = str_replace(
            route('campaign_assets', ['uid' => $this->uid]).'/',
            '',
    ...
    public function copyFromTemplate($template)
    {
        ...
        \App\Library\Tool::xcopy($template->getStoragePath(), $this->getStoragePath());
    ...
    public function occupiedByOtherAnotherProcess()
    {
        if (!function_exists('posix_getpid')) {
            MailLog::warning('posix_getpid() function not available, skip process checking');
            return false;
        }
