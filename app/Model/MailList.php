...
use App\Model\SubscriberFields;
use App\Model\Fields;
use App\Exceptions\VerificationProcessCancelledException;
use DB;
use App;
...
class MailList extends Model
{
    ...
    {
        return $this->hasMany('App\Model\Segment');
    }

    public function automations()
    {
        return $this->hasMany('App\Model\Automation2');
    }
    ...
    {
        return $this->belongsToMany('App\Model\Campaign', 'campaigns_lists_segments', 'mail_list_id', 'campaign_id');
    }
    ...
    public function createDefaultFieds()
    {
        ...
        $this->fields()->create([
                            ...
                            'tag' => \App\Model\Field::formatTag(trans('messages.first_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
        ...
        $this->fields()->create([
                            ...
                            'tag' => \App\Model\Field::formatTag(trans('messages.last_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
    }
    ...
    {
        return \App\Model\SystemJob::where('name', '=', "App\Jobs\ImportSubscribersJob")
    ...
    {
        return \App\Model\SystemJob::where('name', '=', "App\Jobs\ExportSubscribersJob")
            ->where('data', 'like', '%"mail_list_uid":"'.$this->uid.'"%');
    }
    ...
    {
        return \App\Model\SystemJob::where('name', '=', "App\Jobs\ExportSegmentsJob")
    ...
    public static function export($list, $customer, $job)
    {
        ...
        for ($page = 0; $page <= ceil($total / $num); ++$page) { // ceil($total/$num)
            ...
            foreach ($list->subscribers()->skip($page * $num)->take($num)->get() as $key => $item) {
                ...
                $data[] = \App\Library\Tool::arrayToCsv($cols, ',');
    ...
    public static function exportSegments($list, $customer, $job)
    {
        ...
        $headers = $json->fields;
        $segment = \App\Model\Segment::findByUid($json->uid);
    ...
    public function sendSubscriptionConfirmationEmail($subscriber)
    {
        ...
        $layout = \App\Model\Layout::where('alias', 'sign_up_confirmation_email')->first();
        $send_page = \App\Model\Page::findPage($list, $layout);
    ...
    public function sendSubscriptionWelcomeEmail($subscriber)
    {
        ...
        $layout = \App\Model\Layout::where('alias', 'sign_up_welcome_email')->first();
        $send_page = \App\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmail($subscriber)
    {
        ...
        $layout = \App\Model\Layout::where('alias', 'unsubscribe_goodbye_email')->first();
        $send_page = \App\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmailToListOwner($subscriber)
    {
        ...
        $template = \App\Model\Layout::where('alias', 'unsubscribe_notification_for_list_owner')->first();
    ...
    public function sendProfileUpdateEmail($subscriber)
    {
        ...
        $layout = \App\Model\Layout::where('alias', 'profile_update_email')->first();
        $send_page = \App\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    private function readCsv($file)
    {
        try {
            ...
            $encoding = \App\Library\StringHelper::detectEncoding($file);

            if ($encoding == false) {
                MailLog::warning("Cannot detect file's encoding: {$file}");
            } elseif ($encoding != 'UTF-8') {
                MailLog::warning("Convert from {$encoding} to UTF-8");
                \App\Library\StringHelper::toUTF8($file, $encoding);
            } else {
                MailLog::info('File encoding is UTF-8');
                \App\Library\StringHelper::checkAndRemoveUTF8BOM($file);
            }
    ...
    {
        return $this->hasMany('App\Model\MailListsSendingServer');
    }
    ...
    public function getSendingServers()
    {
        ...
        if ($this->customer->getOption('sending_server_option') == \App\Model\Plan::SENDING_SERVER_OPTION_OWN) {
        ...
        } elseif ($this->customer->getOption('sending_server_option') == \App\Model\Plan::SENDING_SERVER_OPTION_SYSTEM) {
            // Check if has sending servers for current subscription
            if (is_object($subscription)) {
                /*
                if ($subscription->plan->getOption('all_sending_servers') == 'yes') {
                    if (\App\Model\SendingServer::getAllAdminActive()->count()) {
                        $result = \App\Model\SendingServer::getAllAdminActive()->get()->map(function ($server) {
                            return [$server->id, '100'];
                        });
                    }
    ...
    public function queueForVerification($serverId)
    {
        ...
        if (is_null($job)) {
            $job = (new \App\Jobs\VerifyMailListJob($this->id, $serverId));
            dispatch($job);
        } else {
    ...
    {
        $job = SystemJob::where('name', 'App\Jobs\VerifyMailListJob')
    ...
    public function subscribe($request, $source)
    {
        ...
        //     if (is_object($subscriber) && $subscriber->status == \App\Model\Subscriber::STATUS_SUBSCRIBED) {
        ...
        event(new \App\Events\MailListUpdated($this));
