...
class Subscription extends Model
{
    ...
    public function plan()
    {
        ...
        return $this->belongsTo('\App\Model\Plan', 'plan_id', 'uid');
    }
    ...
    {
        // @todo dependency injection
        return $this->belongsTo('\App\Model\Customer', 'user_id', 'uid');
    }
    ...
    public function createInitInvoice()
    {
        ...
        $invoice->description = trans('messages.invoice.init_subscription.desc', [
            'plan' => $this->plan->name,
            'date' => \App\Library\Tool::formatDate($this->getPeriodEndsAt(\Carbon\Carbon::now())),
        ]);
    ...
    public function createChangePlanInvoice($newPlan)
    {
        ...
        $invoice->description = trans('messages.change_plan.desc', [
            ...
            'date' => \App\Library\Tool::formatDate(\Carbon\Carbon::parse($metadata['endsAt'])),
        ]);
        $invoice->customer_id = $this->user->id;
        $invoice->currency_id = $this->plan->currency_id;
        $invoice->save();
                ...
                if ($this->isExpiring() && $this->canRenewPlan() && !$this->cancelled()) {
                    ...
                    if (
                        ...
                        $pendingInvoice->addTransaction([
                            'status' => \App\Model\Transaction::STATUS_PENDING,
    ...
    public function reachDueDate()
    {
        return \Carbon\Carbon::now()->greaterThanOrEqualTo(
            $this->current_period_ends_at->subDays(\App\Model\Setting::get('recurring_charge_before_days'))
        );
    }
    ...
    {
        // @todo dependency injection
        return $this->hasMany('\App\Model\SubscriptionLog');
    }
