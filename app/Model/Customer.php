...
use Carbon\Carbon;
use App\Cashier\Interfaces\BillableUserInterface;
...
class Customer extends Model implements BillableUserInterface
{
    ...
    {
        return $this->hasOne('App\Model\Subscription', 'user_id', 'uid')
    ...
    {
        return $this->hasMany('App\Model\MailList')->orderBy('created_at', 'desc');
    }
    ...
    {
        return $this->hasMany('App\Model\Campaign')->orderBy('created_at', 'desc');
    }

    public function sentCampaigns()
    {
        return $this->hasMany('App\Model\Campaign')->where('status', '=', 'done')->orderBy('created_at', 'desc');
    }

    public function subscribers()
    {
        return $this->hasManyThrough('App\Model\Subscriber', 'App\Model\MailList');
    }

    public function logs()
    {
        return $this->hasMany('App\Model\Log')->orderBy('created_at', 'desc');
    }

    public function trackingLogs()
    {
        return $this->hasMany('App\Model\TrackingLog')->orderBy('created_at', 'asc');
    }
    ...
    {
        return $this->hasMany('App\Model\Automation2')->where('status', Automation2::STATUS_ACTIVE);
    }

    public function sendingDomains()
    {
        return $this->hasMany('App\Model\SendingDomain');
    }
    ...
    {
        return $this->hasMany('App\Model\EmailVerificationServer');
    }
    ...
    {
        return $this->hasMany('App\Model\Blacklist');
    }
    ...
    {
        return $this->hasMany('App\Model\TrackingDomain');
    }

    // billing addresses
    public function billingAddresses()
    {
        return $this->hasMany('App\Model\BillingAddress');
    }
    ...
    public static function select2($request)
    {
        ...
        $query = \App\Model\Customer::getAll()->leftJoin('users', 'users.id', '=', 'customers.user_id');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('customers.first_name', 'like', '%'.$keyword.'%')
    ...
    public function updateInformation($request)
    {
        ...
        $user = is_object($this->user) ? $this->user : new \App\Model\User();
    ...
    {
        return $this->hasMany('App\Model\SendingServer');
    }

    public function subAccounts()
    {
        return $this->hasMany('App\Model\SubAccount');
    }
    ...
    public function getSendingDomains()
    {
        ...
        // If customer dont have permission creating sending domain
            ...
            $result = \App\Model\SendingDomain::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        }
    ...
    public function assignPlan($plan)
    {
        ...
        $subscription->pendingInvoices()->delete();
    ...
    public function hasSubscriptionNotice()
    {
        ...
        if ($subscription->renewInvoice() && !$subscription->renewInvoice()->isPaid() && !$subscription->user->supportsAutoBilling()) {
            return true;
        }

        if ($subscription->changePlanInvoice() && !$subscription->changePlanInvoice()->isPaid()) {
            return true;
        }
    ...
    public function getPaymentMethod()
    {
        ...
        return json_decode($this['payment_method'], true);
    }
    ...
    public function getPaymentGateway()
    {
        ...
        return \App\Model\Setting::getPaymentGateway($this->getPaymentMethod()['method']);
    }

    /**
     * Check if payment support recurring.
     *
     * @return boolean
     */
