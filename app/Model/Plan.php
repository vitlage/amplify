...
use App\Cashier\Interfaces\BillablePlanInterface;
...
class Plan extends Model implements BillablePlanInterface
{
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == \App\Model\Plan::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    {
        return $this->hasMany('App\Model\PlansSendingServer');
    }
    ...
    {
        return $this->hasMany('App\Model\Subscription', 'plan_id', 'uid')
    ...
    {
        return $this->hasMany('App\Model\PlansEmailVerificationServer');
    }
    ...
    public static function defaultOptions()
    {
        ...
        foreach (\App\Model\SendingServer::types() as $key => $type) {
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return strtolower(\App\Library\Tool::format_number($this->getOption('sending_quota')).' '.trans('messages.'.\App\Library\Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'.\App\Library\Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota'))));
        }
    }
    ...
    public function displayTotalQuota()
    {
        if ($this->getOption('email_max') == -1) {
            ...
            return \App\Library\Tool::format_number($this->getOption('email_max'));
        }
    }
    ...
    public function displayMaxList()
    {
        if ($this->getOption('list_max') == -1) {
            ...
            return \App\Library\Tool::format_number($this->getOption('list_max'));
        }
    }
    ...
    public function displayMaxSubscriber()
    {
        if ($this->getOption('subscriber_max') == -1) {
            ...
            return \App\Library\Tool::format_number($this->getOption('subscriber_max'));
        }
    }
    ...
    public function displayMaxCampaign()
    {
        if ($this->getOption('campaign_max') == -1) {
            ...
            return \App\Library\Tool::format_number($this->getOption('campaign_max'));
        }
    }
    ...
    public function displayMaxSizeUploadTotal()
    {
        if ($this->getOption('max_size_upload_total') == -1) {
            ...
            return \App\Library\Tool::format_number($this->getOption('max_size_upload_total'));
        }
    }
    ...
    {
        if ($this->getOption('sending_server_option') != \App\Model\Plan::SENDING_SERVER_OPTION_OWN) {
    ...
    public static function select2($request)
    {
        ...
        $query = \App\Model\Plan::getAllActive()->orderBy('custom_order', 'asc');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('plans.name', 'like', '%'.$keyword.'%');
        ...
        // Read all check
        if ($request->user()->admin && !$request->user()->admin->can('readAll', new \App\Model\Plan())) {
            $query = $query->where('plans.admin_id', '=', $request->user()->admin->id);
        }

        if ($request->change_from_uid) {
            $plan = \App\Model\Plan::findByUid($request->change_from_uid);
        ...
        foreach ($query->limit(20)->get() as $plan) {
            $data['items'][] = ['id' => $plan->uid, 'text' => htmlspecialchars($plan->name).'|||'.htmlspecialchars(\App\Library\Tool::format_price($plan->price, $plan->currency->format))];
        }
    ...
    public static function getAllActiveWithDefault($admin = null)
    {
        ...
        if (isset($admin) && !$admin->can('readAll', new \App\Model\Plan())) {
    ...
    public function displayFrequencyTime()
    {
        ...
        return $this->frequency_amount.' '.\App\Library\Tool::getPluralPrase($this->frequency_unit, $this->frequency_amount);
    }
    ...
    public function fillPlansEmailVerificationServers($params)
    {
        ...
        foreach ($params as $key => $param) {
            if ($param['check']) {
                $server = \App\Model\EmailVerificationServer::findByUid($key);
                $row = new \App\Model\PlansEmailVerificationServer();
                $row->plan_id = $this->id;
    ...
    {
        $planMethodlist = \App\Model\PaymentMethod::getAllActive();
    ...
    public function updateFitnesses($hash)
    {
        foreach ($hash as $uid => $value) {
            ...
            \App\Model\PlansSendingServer::where('sending_server_id', '=', $sendingServer->id)
    ...
    public function primarySendingServer()
    {
        if (!$this->useSystemSendingServer()) {
            throw new \Exception('App ERROR: 120000700392');
        }
    ...
    {
        return \App\Library\Tool::format_price($this->price, $this->currency->format);
    }
    ...
    public function getEmailVerificaionServers()
    {
        if ($this->getOption('all_email_verification_servers') == 'yes') {
            $result = \App\Model\EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        } else {
    ...
    {
        return $this->getOption('sending_server_option') == \App\Model\Plan::SENDING_SERVER_OPTION_SYSTEM;
    }

    public function useOwnSendingServer()
    {
        return $this->getOption('sending_server_option') == \App\Model\Plan::SENDING_SERVER_OPTION_OWN;
    }
    ...
    public static function getAvailablePlans()
    {
        ...
        $aPlans = [];
        // $psService = \App\Model\Setting::getPaymentGateway('paypal_subscription');
        foreach ($plans as $plan) {
            if ($plan->isAvailable()) {
                // if (config('cashier.paypal_payment') !== 'paypal_subscription' ||
                //     $psService->findPlanConnection($plan)
                // ) {
                $aPlans[] = $plan;
                // }
            }
