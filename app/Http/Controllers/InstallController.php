...
class InstallController extends Controller
{
    ...
    public function siteInfo(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!empty($request->license)) {
                try {
                    $site_info["license_type"] = \App\Helpers\LicenseHelper::getLicenseType($request->license);
                } catch (\Exception $ex) {
    ...
    public function import(Request $request)
    {
        ...
        // mail setting
        \App\Model\Setting::set('mailer.driver', $site_info['mail_driver']);
        \App\Model\Setting::set('mailer.host', $site_info['smtp_hostname']);
        \App\Model\Setting::set('mailer.port', $site_info['smtp_port']);
        \App\Model\Setting::set('mailer.encryption', $site_info['smtp_encryption']);
        \App\Model\Setting::set('mailer.username', $site_info['smtp_username']);
        \App\Model\Setting::set('mailer.password', $site_info['smtp_password']);
        \App\Model\Setting::set('mailer.from.name', $site_info['mail_from_name']);
        \App\Model\Setting::set('mailer.from.address', $site_info['mail_from_email']);
    ...
    public function checkSystemCompatibility()
    {
        return [
            [
                ...
                'note' => 'PHP 7.2.5 or higher is required.',
            ],
            [
                'type' => 'requirement',
