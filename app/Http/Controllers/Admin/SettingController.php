...
class SettingController extends Controller
{
    ...
    public function general(Request $request)
    {
        ...
        // \App\Model\Setting::updateAll();
        $settings = \App\Model\Setting::getAll();
        if (null !== $request->old()) {
                ...
                if ($name != '_token' && isset($settings[$name])) {
                    ...
                    if ($name == 'site_logo_small' || $name == 'site_logo_big' || $name == 'site_favicon') {
                        if ($request->hasFile($name) && $request->file($name)->isValid()) {
                            \App\Model\Setting::uploadFile($request->file($name), $name, false);
                        }
                    } else {
                        if ($settings[$name]['cat'] == 'general' && $request->user()->admin->getPermission('setting_general') == 'yes') {
                            \App\Model\Setting::set($name, $value);
                        }
                    }
                }
            }
    ...
    public function cronjob(Request $request)
    {
        ...
        if ($request->re_generate_remote_job_url) {
            ...
            \App\Model\Setting::set('remote_job_token', $remote_job_token);
        ...
        if ($request->isMethod('post')) {
            ...
            foreach ($env as $key => $value) {
                ...
                if ($key == 'MAIL_PASSWORD') {
                    \App\Model\Setting::setEnv($key, quoteDotEnvValue($value));
                } else {
                    \App\Model\Setting::setEnv($key, $value);
                }
            }

            // update settings table
            \App\Model\Setting::set('mailer.driver', $env['MAIL_DRIVER']);
            \App\Model\Setting::set('mailer.host', $env['MAIL_HOST']);
            \App\Model\Setting::set('mailer.port', $env['MAIL_PORT']);
            \App\Model\Setting::set('mailer.encryption', $env['MAIL_ENCRYPTION']);
            \App\Model\Setting::set('mailer.username', $env['MAIL_USERNAME']);
            \App\Model\Setting::set('mailer.password', $env['MAIL_PASSWORD']);
            \App\Model\Setting::set('mailer.from.name', $env['MAIL_FROM_NAME']);
            \App\Model\Setting::set('mailer.from.address', $env['MAIL_FROM_ADDRESS']);
    ...
    public function updateUrls(Request $request)
    {
        ...
        \App\Model\Setting::set('url_unsubscribe', action('CampaignController@unsubscribe', ['message_id' => 'MESSAGE_ID']));
        \App\Model\Setting::set('url_open_track', action('CampaignController@open', ['message_id' => 'MESSAGE_ID']));
        \App\Model\Setting::set('url_click_track', action('CampaignController@click', ['message_id' => 'MESSAGE_ID', 'url' => 'URL']));
        \App\Model\Setting::set('url_delivery_handler', action('DeliveryController@notify', ['stype' => '']));
        \App\Model\Setting::set(
            'url_update_profile',
        ...
        \App\Model\Setting::set('url_web_view', action('CampaignController@webView', ['message_id' => 'MESSAGE_ID']));
    ...
    public function license(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                // Update license type
                \App\Model\Setting::updateLicense($request->license);
        ...
        return view('admin.settings.license', [
            'settings' => $settings,
            'current_license' => \App\Model\Setting::get('license'),
    ...
    public function uploadApplicationPatch(Request $request)
    {
        ...
        try {
            ...
            if (!$supported) {
                $request->session()->flash('alert-error', sprintf('Your App Mail SUPPORT was already expired on <strong>%s</strong>. Please <a style="text-decoration:underline;font-style: normal;" target="_blank" href="%s">renew</a> it first before proceeding with our LIVE upgrade service.', $supportedUntil->toFormattedDateString(), 'https://codecanyon.net/item/App-email-marketing-web-application/17796082'));
            ...
            foreach ($request->all() as $name => $value) {
                if ($name != '_token' && isset($settings[$name])) {
                    ...
                    if ($name == 'site_logo_small' || $name == 'site_logo_big') {
                        if ($request->hasFile($name) && $request->file($name)->isValid()) {
                            \App\Model\Setting::uploadSiteLogo($request->file($name), $name);
                        }
                    } else {
    ...
    public function advancedUpdate(Request $request, $name)
    {
        ...
        \App\Model\Setting::set($name, $request->value);
    ...
    public function payment(Request $request)
    {
        ...
        // Save settings
        \App\Model\Setting::set('end_period_last_days', $request->end_period_last_days);
        \App\Model\Setting::set('renew_free_plan', $request->renew_free_plan);
        \App\Model\Setting::set('recurring_charge_before_days', $request->recurring_charge_before_days);
