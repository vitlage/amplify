...
class PaymentController extends Controller
{
    ...
    {
        $service = \App\Model\Setting::getPaymentGateway($name);
        $gateway = Setting::getPayment($name);

        // authorize
        if (\Gate::denies('update', new \App\Model\PaymentMethod())) {
            return $this->notAuthorized();
        }
    ...
    public function update(Request $request, MessageBag $message_bag, $name)
    {
        ...
        $gatewayService = \App\Model\Setting::getPaymentGateway($name, $request->options);
