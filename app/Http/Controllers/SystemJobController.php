...
class SystemJobController extends Controller
{
    ...
    {
        $system_jobs = \App\Model\SystemJob::search($request)->paginate($request->per_page);
    ...
    {
        $job = \App\Model\ImportSubscribersSystemJob::find($request->id);

        // authorize
        // @todo: redundant find() here
        if (\Gate::denies('downloadImportLog', \App\Model\SystemJob::find($request->id))) {
    ...
    {
        $jobs = \App\Model\SystemJob::whereIn('id', explode(',', $request->uids));
    ...
    {
        $job = \App\Model\SystemJob::find($request->id);
