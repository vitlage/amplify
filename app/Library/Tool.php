...
class Tool
{
    ...
    public static function cronjobUpdateController($request, $controller)
    {
        ...
        $setting_php_bin_path = \App\Model\Setting::get('php_bin_path');
        ...
        if (empty(\App\Model\Setting::get('remote_job_token'))) {
            \App\Model\Setting::set('remote_job_token', str_random(60));
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->queue_driver == 'database') {
                ...
                \App\Model\Setting::set('php_bin_path', $php_bin_path_value);
            ...
            if (in_array($queue_driver, ['database', 'async']) && config('queue.default') != $queue_driver) {
                \App\Model\Setting::setEnv('QUEUE_DRIVER', $queue_driver);
            }
    ...
    public static function replaceTemplateUrl($content, $path)
    {
        ...
        foreach ($srcs as $src) {
            ...
            if (preg_match('/https?/i', $src) || preg_match('/^\{/i', $src) || preg_match('/^\%/i', $src) || preg_match('/^https?/i', $src) || preg_match('/^#/i', $src) || strpos($src, '//') === 0 || preg_match('/^mailto:/i', $src) || preg_match('/^tel:/i', $src)) {
                ...
                // if it is an absolute path, like /files/208394398/some-thing.jpg
                // then do nothing
                // this is for the case when file is added via file manager
            } else {
                // Build the full path
                $new_src = join_url($path, $new_src);
            }
