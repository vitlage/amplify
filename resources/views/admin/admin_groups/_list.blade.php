        ...
        @foreach ($groups as $key => $item)
            <tr>
                ...
                <td class="text-right">
                    ...
                    @can('update', $item)
                        <a href="{{ action('Admin\AdminGroupController@edit', $item->id) }}" type="button" class="btn bg-grey-600 btn-icon btn-fixed"><i class="icon icon-pencil"></i> {{ trans('messages.edit') }}</a>
                    @endcan
                    @can('delete', $item)
                        <div class="btn-group">
