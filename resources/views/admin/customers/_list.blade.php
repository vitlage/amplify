            ...
            <tr>
                ...
                <td class="stat-fix-size">
                    @if (is_object($item->subscription) && !$item->subscription->isEnded())
                        <div class="single-stat-box pull-left ml-10">
                ...
                <td class="text-right" style="display: flex;">
                    @can('loginAs', $item)
                        <a href="{{ action('Admin\CustomerController@loginAs', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.login_as_this_customer') }}" type="button" class="btn bg-teal-601 btn-icon"><i class="icon icon-enter"></i></a>
                    @endcan
                    @can('update', $item)
                        <a href="{{ action('Admin\CustomerController@edit', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" type="button" class="btn bg-grey-600 btn-icon btn-fixed"><i class="icon icon-pencil pr-0 mr-0"></i></a>
                    @endcan
